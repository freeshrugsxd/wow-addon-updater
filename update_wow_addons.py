...
class Updater:
    def __init__(self):
        ...
        if not isfile(self.config_file):
            exit(f'Error: Config file \'{self.config_file}\' not found.')

        with open(self.config_file, 'r') as f:
            self.config.read_file(f)

        self.game_version = self.config['settings']['version'].lower()
        if self.game_version not in ['classic', 'retail']:
            exit(f'Error: Game version \'{self.game_version}\' not recognized.'
                 f'Must be either \'classic\' or \'retail\'.')

        self.game_dir = self.config['settings']['game directory']
        if not isdir(self.game_dir):
            exit(f'Error: \'{self.game_dir}\' is not a directory.')

        self.addon_dir = pjoin(self.game_dir, f'_{self.game_version}_', 'Interface', 'AddOns')
        if not isdir(self.addon_dir):
            exit(f'Error: \'{self.addon_dir}\' is not a directory.')

        self.cache_dir = pjoin(expanduser('~'), '.cache', 'wow-addon-updates')
        if not isdir(self.cache_dir):
            mkdir(self.cache_dir)

        self.addons = Manager().dict({})

        for name, last_update in self.config.items(self.game_version):
            if last_update is None or self.overwrite:
                last_update = 0.0
    ...
    def main(self):
        ...
        with Pool(num_workers) as p:
            p.map(self.find_update, self.addons)
        ...
        if updateable_len == 0:
            ...
            with Pool(num_workers) as p:
                for _ in tqdm(p.imap_unordered(self.update_addon, updateable), total=updateable_len):
                    pass

            for addon_name, timestamp in updated.items():
                self.config[self.game_version][addon_name] = str(timestamp)
